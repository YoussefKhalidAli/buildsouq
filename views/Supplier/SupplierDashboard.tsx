...
export const SupplierDashboard = () => {
  const { products, categories, currentUser, addProduct, updateProduct, deleteProduct } = useStore();
  ...
  const lowStockProducts = myProducts.filter(p => p.stock <= (p.lowStockThreshold || 10));
        ...
        <div className="mb-8 bg-amber-50 border border-amber-200 rounded-xl p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {lowStockProducts.map(p => (
              <div key={p.id} className="bg-white p-3 rounded-lg border border-amber-100 flex justify-between items-center shadow-sm">
                <div>
                  <div className="text-sm font-medium text-slate-900">{p.name}</div>
                  <div className="text-xs text-slate-500">Current Stock: <span className="text-red-600 font-bold">{p.stock}</span></div>
                </div>
            ...
            {myProducts.length === 0 ? (
              ...
              myProducts.map(product => (
                <tr key={product.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-right font-medium">{formatMoney(product.price)}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex flex-col items-end gap-1">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${product.stock > (product.lowStockThreshold || 10) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {product.stock}
                      </span>
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    ...
                    <button onClick={() => { if(confirm('Delete this product?')) deleteProduct(product.id) }} className="text-red-600 hover:text-red-800 p-1">
