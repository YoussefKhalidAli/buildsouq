...
export const SuperAdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "users" | "catalog" | "suppliers" | "orders"
  >("users");
  ...
  const [sortKey, setSortKey] = useState<
    "name" | "role" | "status" | "expiration" | "suspended"
  >("name");
  ...
  const filteredUsers = users
    ...
    .filter((u) => {
      ...
      if (currentUser?.role === "logistics-super-admin")
  ...
  const RoleBadge = ({ role }: { role: string }) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter ${config.bg} ${config.text}`}
      >
  ...
  const StatusBadge = ({ verified }: { verified: boolean }) => (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-black uppercase ${verified ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
    >
      {verified ? (
    ...
    <div className="container mx-auto px-4 py-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex bg-white p-1 rounded-xl shadow-sm border border-slate-200">
          ...
          {(currentUser?.role === "superadmin" ||
            currentUser?.role === "logistics-super-admin") && (
            <button
              onClick={() => setActiveTab("orders")}
              className={`px-6 py-2.5 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === "orders" ? "bg-slate-900 text-white shadow-lg" : "text-slate-500 hover:text-slate-900"}`}
            >
              Orders
            </button>
          )}
              ...
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th
                    className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500 cursor-pointer"
                    onClick={() => {
                      if (sortKey === "expiration") setSortAsc(!sortAsc);
                      else {
                        setSortKey("expiration");
                        setSortAsc(true);
                      }
                    }}
                  >
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white shadow-sm ${user.verified ? "bg-slate-900" : "bg-slate-400"}`}
                        >
                          {user.name.charAt(0)}
                        </div>
                        <div>
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {categories.map((cat) => (
              <div
                key={cat.id}
                className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-blue-400 hover:bg-white transition-all"
              >
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-6 py-4 text-left font-bold text-slate-500 uppercase tracking-widest text-[10px]">
                  Company
                </th>
                <th className="px-6 py-4 text-center font-bold text-slate-500 uppercase tracking-widest text-[10px]">
                  Status
                </th>
                <th className="px-6 py-4 text-right font-bold text-slate-500 uppercase tracking-widest text-[10px]">
                  Action
                </th>
              </tr>
            </thead>
              ...
              {suppliers.map((supplier) => (
                <tr key={supplier.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase ${supplier.active ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                    >
        ...
        {selectedUser && (
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              {selectedUser.role === "supplier-admin" ||
              selectedUser.role === "delivery" ? (
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(selectedUser.documents || {}).map(
                    ([key, value]) => (
                      <div
                        key={key}
                        className="flex items-center gap-4 p-4 bg-white border-2 border-slate-100 rounded-2xl"
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-[10px] text-emerald-600 font-bold flex items-center gap-1">
                            <CheckCircle2 className="w-3 h-3" /> Securely
                            Attached
                          </div>
                        </div>
                        <a
                ...
                <div className="bg-slate-50 p-12 rounded-2xl text-center border-2 border-dashed border-slate-200">
                  ...
                  <p className="text-sm text-slate-500 font-medium italic">
                    Basic Customer Account: No corporate documents required.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">
                    The account will be suspended automatically when the
                    certificate expires.
                  </p>
                </div>
                <div>
                    ...
                    {!selectedUser.verified ? (
                      <Button
                        ...
                        onClick={async () => {
                          if (!expirationDate) {
                            setReviewError(
                              "Please set an expiration date before verifying.",
                            );
                            return;
                          }
                          setReviewError(null);
